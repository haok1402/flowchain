...
const ThirdPartyAuthenticator = React.memo(() => {
  const { theme } = useTheme();
  const { continueWithGoogle } = useFirebase();
  const { continueWithGitHub } = useFirebase();
  const [showLightGithub, setShowLightGithub] = useState(theme === "light");
  return (
    <>
      ...
      <div className="mb-3">
        <Button
          style={{ width: "100%" }}
          variant={`outline-${theme === "light" ? "dark" : "light"}`}
          onMouseEnter={() => setShowLightGithub(theme !== "light")}
          onMouseLeave={() => setShowLightGithub(theme === "light")}
...
const FirstPartyAuthenticator = React.memo(() => {
  const { theme } = useTheme();
